...
class _IndexPageState extends State<IndexPage> {
  ...
  List<Widget> pages = <Widget>[];
   ...
   Widget appbarTitile(int index, BuildContext context){
     if(index == 0){
       return Row(
           ...
           ChangeNotifierProvider(
             ...
             child: Consumer<Data>(
               builder: (contexts, model, child){
                 Data data = Provider.of<Data>(context, listen: true);
   ...
   List<Widget> appBarActions(int index, BuildContext context){
     if(index == 0){
  ...
  void initState() {  // TODO: implement initState3
    super.initState();
    WidgetsBinding.instance?.addPostFrameCallback((_){ //build完成时调用
      //开启socket
      SocketConnect.connects(context, reConnect: true, duration: const Duration(seconds: 5));
    });
    pages
      ..add(HomePage())
      ..add(MinePage());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        elevation: 0,
        title: appbarTitile(_currentIndex, context),
        actions: appBarActions(_currentIndex, context),
      ),
